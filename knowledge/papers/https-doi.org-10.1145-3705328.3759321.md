...
> 论文揭示了推荐系统框架RecBole中隐藏的默认早停策略会显著影响超参数优化结果，导致性能差异，强调了框架透明性和可复现性的重要性。
...
超参数优化对于提升推荐系统的性能至关重要，但其实现常被视为中性或次要问题。在本工作中，我们将关注点从模型基准测试转向审计广泛使用的推荐框架RecBole的行为。我们证明，RecBole的内部默认设置，特别是未文档化的早停策略，可能过早终止随机搜索和贝叶斯优化。这以用户不可见的方式限制了搜索覆盖范围。通过使用六个模型和两个数据集，我们比较了搜索策略，并量化了性能方差与搜索路径的不稳定性。我们的发现表明，隐藏的框架逻辑可能引入与搜索策略间差异相当的可变性。这些结果凸显了将框架视为实验设计主动组成部分的重要性，并呼吁在推荐系统研究中采用更透明、更注重可复现性的工具。我们为研究人员和开发者提供了可操作的建议，以减轻隐藏配置行为的影响，并提升超参数调优工作流的透明度。
...
_知识库更新时间：2026-07-24T04:05:55.200887_
