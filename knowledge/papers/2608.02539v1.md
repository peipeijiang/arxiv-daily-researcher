...
> 该论文提出了一种岭回归估计量有限样本分布的简单高斯近似，并基于此提出了两种正则化参数选择策略。
...
我们提出了一种简单的高斯近似，用于经典岭回归估计量的有限样本分布。我们的近似捕捉了这样一个事实：在有限样本中，岭回归估计量通过权衡偏差与方差来降低估计误差和预测误差。该近似基于非标准渐近理论，其中：$i)$ 我们让估计量的正则化参数随样本量成比例增长；$ii)$ 我们将总体回归系数视为相对于定义估计量收缩方向的参考向量的“局部”（local）参数。与文献中其他渐近近似不同，我们允许数据生成过程中存在一般形式的异方差性和自相关性（代价是考虑低维模型，其中协变量数量不允许随样本量增长）。我们利用这一简单的高斯近似，提出了两种新的策略来选择岭回归估计量的正则化参数。所建议的策略通过最小化平均或最坏情况下的超额预测风险（excess prediction risk）来选择正则化参数，其中风险是基于我们提出的高斯近似计算的。
...
_知识库更新时间：2026-08-04T04:04:06.461418_
