...
paper_id: "https://doi.org/10.5281/zenodo.21304820"
source: "openalex"
...
[查看原文](https://doi.org/10.5281/zenodo.21304820)

## 一句话结论

> 针对BPR忽略用户兴趣局部集中性的问题，提出LBN-BPR算法，通过局部邻域构建和局部贝叶斯优化，提升隐式反馈下的个性化排序性能。
...
- **发布时间**：2026-07-11
- **相关度评分**：36.0
- **DOI**：[https://doi.org/10.5281/zenodo.21304820](https://doi.org/10.5281/zenodo.21304820)

<details open>
<summary><strong>中文摘要</strong></summary>

在信息爆炸的时代，推荐系统已成为缓解信息过载问题的核心技术。隐式反馈（如点击、浏览和购买）因其易于收集的特性，已成为训练推荐模型的主要数据来源。贝叶斯个性化排序（Bayesian Personalized Ranking, BPR）作为一种经典的成对偏好排序算法，在处理隐式反馈方面展现出卓越性能。然而，传统BPR在参数优化过程中采用全局采样与全局更新策略，这往往忽略了用户兴趣的局部集中性。因此，模型极易受到全局稀疏噪声的影响，导致其捕获细粒度局部化用户偏好的能力不足。针对上述局限，本文提出了一种名为局部贝叶斯邻域BPR（Local Bayesian Neighborhood BPR, LBN-BPR）的新型推荐算法。该算法的核心思想是严格在用户的局部高相似邻域内定义并优化后验概率，从而使训练过程免受全局噪声干扰。LBN-BPR首先利用先进的语义相似度度量方法为每位用户构建局部邻域空间，并在此基础上建立局部贝叶斯优化框架。通过约束随机梯度下降（Stochastic Gradient Descent, SGD）的更新范围，模型参数得以专注于训练与目标用户具有相似行为特征的局部子群体。理论分析与梯度推导表明，LBN-BPR能够以更高精度拟合局部化的个性化偏好。本文详细阐述了LBN-BPR的理论基础、建模过程、优化策略及复杂度分析，为隐式反馈场景下的高质量个性化推荐提供了一种鲁棒且抗噪声的解决方案。
...
_知识库更新时间：2026-07-12T10:37:28.672664_
