...
[查看原文](https://dblp.org/rec/conf/cikm/0009ZYFPG25)

## 一句话结论

> 提出双上下文感知负采样策略（DCANS），通过评估正样本与用户兴趣的对齐度并调整合成负样本的难度，缓解隐式反馈中的假阳性问题，提升推荐性能。
...
负采样在协同过滤（collaborative filtering, CF）中扮演着关键角色，因为它能加速收敛并提升推荐精度。在近期研究中，基于混合（mixup）的负采样方法展现出了良好的性能。然而，现有方法主要关注增加合成负样本与正样本之间的相似性，而未考虑隐式反馈场景中常见的假阳性问题。盲目地使用过强负样本训练所有正样本，会放大假阳性的影响，从而损害推荐性能。为应对这一挑战，我们首先通过理论分析揭示：混合合成的强负样本会隐式地重新加权用户交互与正负边界之间的相似性差异，从而塑造训练信号。受此启发，我们提出了一种名为双上下文感知负采样（Dual Context-Aware Negative Sampling, DCANS）的新策略，该策略通过评估每个正样本与用户兴趣上下文的对齐程度来增强正样本，同时根据合成负样本与同一兴趣上下文的相关性来调整其难度。该策略将训练方向优化至用户的真实偏好，在保留强负采样优势的同时减轻了假阳性的负面影响。在三个基准数据集上的大量实验表明，我们的方法相较于现有最优基线取得了持续改进。我们的PyTorch实现已公开于https://github.com/Wu-Xi/DCANS。
...
_知识库更新时间：2026-07-19T04:08:52.139056_
