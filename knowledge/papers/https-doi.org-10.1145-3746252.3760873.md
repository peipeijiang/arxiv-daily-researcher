...
- DOI: https://doi.org/10.1145/3746252.3760873
- URL: https://dblp.org/rec/conf/cikm/0040WXL25
- Venue: CIKM
- Score: 40.0
...
- [WANGLin0126/CLSRec](https://github.com/WANGLin0126/CLSRec) - official (85 confidence), 1 stars

## Deep Analysis

```json
{
  "chinese_title": "社交关系遇见推荐：增强与对齐",
  "summary": "推荐系统对现代内容平台至关重要，但传统基于行为的模型在处理交互数据有限的冷启动用户时面临挑战。为弥补这一差距，本文提出利用社交关系图来丰富基于行为模型的兴趣表示。然而，从社交图中提取价值面临关系噪声和跨领域不一致性的挑战。为解决噪声传播并获得准确的社交兴趣，作者采用双视图去噪策略，对用户-物品交互矩阵进行低秩SVD以得到去噪的社交图，并通过对比学习对齐原始和重构的社交图。针对社交兴趣与行为兴趣之间的不一致性，采用“互蒸馏”技术将原始兴趣分离为对齐的社交/行为兴趣以及社交/行为特定兴趣，最大化两者的效用。在广泛采用的工业数据集上的实验结果验证了该方法的有效性，尤其对冷启动用户，为未来研究提供了新视角。代码已公开。",
  "innovations": [
    "提出双视图去噪策略，利用低秩SVD和对比学习减少社交关系噪声。",
    "采用互蒸馏技术分离对齐兴趣和特定兴趣，解决社交与行为兴趣不一致问题。",
    "有效提升冷启动用户的推荐性能。"
  ],
  "methodology": "首先，对用户-物品交互矩阵进行低秩SVD，得到去噪的社交图；然后，通过对比学习对齐原始社交图和重构社交图。接着，使用互蒸馏技术将原始兴趣分离为对齐的社交/行为兴趣和社交/行为特定兴趣。最后，在工业数据集上评估模型性能。",
  "key_results": "实验结果表明该方法有效，尤其对冷启动用户表现良好。具体数值摘要未提供。",
  "tech_stack": [
    "低秩SVD",
    "对比学习",
    "互蒸馏"
  ...
  "limitations": "摘要未提供具体局限性分析。当前证据仅基于摘要，无法评估实验细节、数据集规模、基线对比等。",
  "relevance_to_keywords": "论文涉及推荐系统、用户建模（冷启动用户）、社交关系利用，与序列推荐、用户建模等关键词相关。但摘要未提及生成式推荐、LLM、多模态等方向。",
