...
- Authors: X.-G. Xia, Saurabh Vishwas Joshi, Kousik Rajesh, Kangnan Li, Yangyi Lu, Nikil Pancha, Dhruvil Badani, Jiajing Xu, Pong Eksombatchai
- DOI: https://doi.org/10.1145/3746252.3761433
- URL: https://dblp.org/rec/conf/cikm/0007JRLLPB0E25
- Venue: CIKM
- Score: 60.0
...
```json
{
  "chinese_title": "TransAct V2：Pinterest推荐中的终身用户行为序列建模",
  "summary": "本文介绍了TransAct V2，一个用于Pinterest首页推荐的生产级模型。该模型通过三个关键创新改进了点击率（CTR）预测：利用超长用户序列、集成下一动作损失函数以及采用可扩展的低延迟部署方案。TransAct V2结合实时和终身用户序列，通过下一动作预测任务增强对用户偏好的理解，并提升推荐多样性。系统设计高效处理长达10^4的终身序列，同时保持低延迟、低存储和低网络成本。离线与在线A/B实验表明，该模型在用户参与度和推荐多样性等关键指标上取得了显著提升，目前已服务于Pinterest超过5亿用户。",
  "innovations": [
  ...
  "methodology": "论文采用基于Transformer的序列建模方法。首先，使用候选物品作为锚点，对终身序列、实时序列和曝光序列进行近邻搜索，并保留最近动作。然后，将子序列拼接后输入Transformer编码器，结合位置编码和特征嵌入（动作类型、表面、PinSage嵌入）。模型采用多任务学习架构，主任务为CTR预测（加权交叉熵损失），辅助任务为下一动作预测（对比损失）。训练时使用负采样和样本加权。部署时通过离线特征缓存、量化（fp16→int8）和近邻搜索优化延迟。",
  "key_results": [
    "在线A/B实验显示，用户参与度指标（如点击率、保存率）显著提升。",
    "推荐多样性指标（如类别覆盖率）得到改善。",
    "离线消融实验验证了终身序列、下一动作损失和部署优化的各自贡献。",
    "模型成功部署于Pinterest首页推荐，服务超过5亿用户。"
  ],
  "tech_stack": [
    "Transformer",
  ...
  "relevance_to_keywords": [
    "序列推荐：核心研究内容，建模用户行为序列。",
    "CTR预测：主要任务，通过序列建模提升点击率预测。",
    "用户建模：通过终身序列和实时序列建模用户兴趣。",
    "工业落地：论文详细描述了部署优化和实际效果。",
    "排序与重排：应用于推荐系统的排序阶段。"
  ]
}
```
