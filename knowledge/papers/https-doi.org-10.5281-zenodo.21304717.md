...
paper_id: "https://doi.org/10.5281/zenodo.21304717"
source: "openalex"
...
提出后验差异贝叶斯个性化排序（PD-BPR），通过将用户偏好建模为高斯分布并优化后验差异梯度，缓解了数据稀疏和长尾问题，提升了排序鲁棒性和泛化能力。

## Metadata

- Authors: Jincheng Zhang
- DOI: https://doi.org/10.5281/zenodo.21304717
- URL: https://doi.org/10.5281/zenodo.21304717
...
推荐系统日益依赖隐式反馈（如点击和购买），这类数据虽然丰富，但本质上存在噪声且非同步。经典的贝叶斯个性化排序（Bayesian Personalized Ranking, BPR）框架通过基于正样本与负样本预测分数之差的绝对值来优化成对排序目标，从而应对这一问题。然而，经典BPR将用户偏好视为确定性标量值，忽略了潜在的预测不确定性与置信水平。因此，在处理高度稀疏或长尾数据时，它容易出现过拟合与梯度消失问题。为克服这些局限，本文提出后验差BPR（Posterior-Difference BPR, PD-BPR），这是一种新颖的优化框架，将范式从确定性点空间排序转变为概率分布空间对齐。PD-BPR并非优化标量化的物品对差值，而是优化“后验差的梯度”。通过将用户偏好建模为高斯后验分布（由期望偏好（均值）与不确定性水平（方差）共同表征），该算法将预测置信度直接融入梯度更新过程。理论分析表明，所推导的梯度具有逆米尔斯比率（Inverse Mills Ratio）与方差驱动的缩放特性，从而建立了一种优雅的自适应调节机制：对于高不确定性物品（如冷启动物品），自适应减小梯度步长以抑制噪声；而对于高置信度预测，则充分释放梯度。此外，均值与方差参数的联合演化提供了软边界缓冲，防止梯度过早饱和。理论分析证明，PD-BPR在严重数据稀疏条件下显著增强了模型的鲁棒性与泛化能力，为隐式反馈推荐场景中稳定成对排序提供了一条数学上严谨的路径。
