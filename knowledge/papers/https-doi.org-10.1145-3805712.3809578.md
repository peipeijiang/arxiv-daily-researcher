...
- Authors: Yifan Liu, Yaokun Liu, Zelin Li, Zhenrui Yue, Gyuseok Lee, Ruichen Yao, Yang Zhang
- DOI: https://doi.org/10.1145/3805712.3809578
- URL: https://doi.org/10.1145/3805712.3809578
...
```json
{
  "chinese_title": "从预训练和协同信号中学习分解的上下文令牌表示用于生成式推荐",
  "summary": "本文针对生成式推荐中两阶段范式（预训练分词器+LLM推荐器）的目标不一致问题，提出DECOR框架。该框架通过分解嵌入融合（保留预训练语义嵌入并融合可学习的协同嵌入）和上下文令牌组合（根据用户交互历史动态调整静态令牌嵌入）两个模块，解决了静态分词导致的表示瓶颈和预训练语义被丢弃的问题。在三个真实数据集上的实验表明，DECOR在推荐性能上持续优于现有基线，且收敛速度更快。",
  "innovations": [
  ...
  "methodology": "采用两阶段生成式推荐范式：首先使用RQ-VAE对预训练语义嵌入进行量化得到语义ID，然后训练T5模型进行自回归生成。DECOR在推荐器训练阶段集成两个模块：分解嵌入融合通过投影和融合层整合预训练代码本嵌入与协同嵌入；上下文令牌组合通过注意力池化从历史序列提取上下文向量，并基于该向量对候选令牌集进行软组合，最后通过残差连接与静态嵌入融合。",
  "key_results": "在Scientific、Instrument、Game三个数据集上，DECOR在Recall@5/10和NDCG@5/10上均优于所有基线。相比最强基线，NDCG@10相对提升在Scientific上超过14%，在Instrument上约5%，在Game上约3%。",
  "tech_stack": [
    "RQ-VAE",
    "T5",
    "Sentence-T5",
  ...
  "relevance_to_keywords": "论文聚焦生成式推荐和序列推荐，提出结合预训练语义与协同信号的方法，与LLM与推荐系统结合、用户建模高度相关。",
